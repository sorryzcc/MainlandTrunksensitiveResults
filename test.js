...
// 检查 Translate 列是否包含黑名单内容并且不包含白名单内容
function checkTranslateContent(item) {
    ...
    for (let black of blackList) {
        if (translateStr.includes(black)) {
            ...
            if (!containsWhite) {
                return true; // 包含黑名单内容但不包含白名单内容
            }
        }
    }
    return false; // 不包含黑名单内容或者也包含了白名单内容
}

// 过滤数据
const result = combinedData.filter(checkTranslateContent);
...
// 写入 Excel 文件
XLSX.writeFile(workbook, 'whiteBalck.xlsx');
